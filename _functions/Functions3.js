...
async function processAddArmour(AddRemove, armorAdd, srcNm, prefix="") {
	...
	if (armorAdd.select) {
		if (!AddRemove) { // remove
			RemoveArmor(armorAdd.select, prefix);
...
async function ApplyMagicItem(input, FldNmbr, field) {
	...
	if (aMI && aMI.choices && !newMIvar) {
		if (parseResult[2].length) {
			...
			if (!selectMIvar && !IsNotImport) {
				...
				aMIvar = aMI[newMIvar];
				field.setValPrepared = aMIvar.name ? aMIvar.name : aMI.name + " [" + selectMIvar + "]";
			}
		} else if (!IsNotImport) {
			failedChoice = true;
		} else {
	...
	if (!aMIvar) {
		...
		var theMI = {
			name : aMIvar.name ? aMIvar.name : field.setValPrepared ? field.setValPrepared : input
		}
...
async function AddMagicItem(item, attuned, itemDescr, itemWeight, overflow, forceAttunedVisible, sItemNote, bIgnoreCurrent, fldName) {
	...
	for (var n = 1; n <= 2; n++) {
		for (var i = startFld; i <= FieldNumbers.magicitems; i++) {
			...
			if (n === 1 && !bIgnoreCurrent && ((RegExItem.test(curItem) && !RegExItemNo.test(curItem)) || curItem.toLowerCase() === itemLower)) {
				...
				if (applyAttunement) await ApplyAttunementMI(i);
				return;
			}
		}
	}
...
async function selectMagicItemGearType(AddRemove, FldNmbr, typeObj, oldChoice, correctingDescrLong, field) {
	...
	if (!AddRemove) {
		...
		if (isApplyFld && field.setValPrepared) selectedItem = baseList[isItem].name;
	...
	if (AddRemove && isApplyFld) {
		...
		if (newMIname !== field.value) field.setValPrepared = newMIname;
	}
}
