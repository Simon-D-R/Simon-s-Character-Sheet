...
async function ApplyWildshape(fldName, value, oldValue) {
	...
	var scores = [
		...
		theCrea.scores[1],
		theCrea.scores[2],
		wasm_character.get_ability("Int"),
		wasm_character.get_ability("Wis"),
		wasm_character.get_ability("Cha")
	];
...
function getAbiModValue(ability, prefix, wildshapeNo, returnScore) {
	...
	if (!prefix) {
		...
		score = wasm_character.get_ability(abi);
	} else if (wildshapeNo) {
...
function EvalDmgDie(input, prefix, isSpecial, useProfB) {
	...
	if ((/^(?=.*(B|C|Q))(?=.*d\d).*$/).test(input)) { //if this involves a cantrip calculation
		...
		var cLvl = !prefix || prefix === true || (isSpecial && isSpecial !== "test") ? wasm_character.get_level() :  Number(What(prefix + "Comp.Use.HD.Level"));
...
async function AskUserOptions(optType, optSrc, optSubj, knownOpt, notProficiencies, sBottomMsg, bReturnIndex) {
...
async function processToNotesPage(AddRemove, items, type, mainObj, parentObj, namesArr) {
	...
	switch (GetFeatureType(type)) {
		case "classes":
			...
			var objClass = {
				name: adapter_helper_get_class_property(namesArr[3], "name"),
				subname: adapter_helper_get_class_property(namesArr[3], "subname"),
				nameShort: adapter_helper_get_class_property(namesArr[3], "nameShort"),
				subnameShort: adapter_helper_get_class_property(namesArr[3], "subnameShort")
			};
