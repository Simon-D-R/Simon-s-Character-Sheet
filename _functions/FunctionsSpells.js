...
function DefineSpellSheetDialogs(force, formHeight) {
	...
	spDias.spellsPrepared = {
		...
		initialize : function (dialog) {
			...
			var abiMod = this.fixedPrepMod ? this.fixedPrepMod : wasm_character.get_ability_modifier(AbilityScores.abbreviations[this.ability - 1]);
...
async function GenerateSpellSheet(GoOn) {
	...
	for (var i = 0; i < CurrentCasters.incl.length; i++) {
		...
		for (var lvl = 0; lvl <= orderedSpellList.length; lvl++) {
			...
			if (needSpace > (lineMax - lineCurrent + 1) || headerCurrent > 3 || dividerCurrent > 9) await AddPage();
